        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                if(packet.d.guild_id === undefined) {
                    ...
                    if(packet.d.channel_id === null) {
                        const flag = this.client.privateChannels.find((privateChannel) => {
            ...
            case "CHANNEL_UPDATE": {
                ...
                if(oldType === packet.d.type) {
                    ...
                    guild.channels.add(newChannel, this.client, true);
                    channel = newChannel;
                }
                ...
                if(guild.pendingVoiceStates && guild.pendingVoiceStates.length > 0) {
                    for(const voiceState of guild.pendingVoiceStates) {
                        ...
                        if(channel) {
                            ...
                            if(this.client.options.seedVoiceConnections && voiceState.id === this.client.user.id && !this.client.voiceConnections.get(guild.id)) {
            ...
            case "READY": {
                ...
                packet.d.guilds.forEach((guild) => {
                    if(!guild.unavailable) {
                        return this.client.unavailableGuilds.remove(this.createGuild(guild));
                    }
                ...
                packet.d.private_channels.forEach((channel) => {
                    if(channel.type === undefined || channel.type === ChannelTypes.DM) {
                        ...
                        return this.client.privateChannels.add(channel, this.client, true);
                    }
